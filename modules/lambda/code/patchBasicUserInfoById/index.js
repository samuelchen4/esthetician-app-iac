...
exports.handler = async (event) => {
  try {
    console.log('patchBasicUserInfoById method start!');
    console.log('Received event:', JSON.stringify(event, null, 2));
    console.log('DB successfully connected!');

    const { userId } = event.pathParameters;
    const { firstName, lastName, email, role } = JSON.parse(event.body);
    ...
    const query = `
        ...
        ;
      `;
    const values = [userId, firstName, lastName, email, role];
